...
class m_pemeliharaan_meter_air extends CI_Model {
	...
	public function getSelect($id){
		...
		SELECT * FROM pemeliharaan_meter_air where active =1 and project_id = $project->id and id = $id
	...
	public function cek($id){
        ...
        $query = $this->db->query("
            SELECT * FROM pemeliharaan_meter_air WHERE id = $id 
        ");
        $row = $query->row();
	...
	public function get_log($id){
        ...
        $query = $this->db->query("
            SELECT 
                code as [Kode],
                ukuran_pipa as [Ukuran Pipa (Dim)],
                harga as [Harga Sewa],
                case 
	...
	public function save($dataTmp)
    {
        ...
        [
			...
			'ukuran_pipa'       => $this->m_core->currency_to_number($dataTmp['ukuran_pipa']),
        ...
        $this->db->from('pemeliharaan_meter_air');

        // validasi double
        if($this->db->count_all_results()==0){ 
            
            $this->db->insert('pemeliharaan_meter_air', $data);
            $idTMP = $this->db->insert_id();
            $dataLog = $this->get_log($idTMP);
            $this->m_log->log_save('pemeliharaan_meter_air', $idTMP, 'Tambah', $dataLog);
	...
	public function edit($dataTmp){
        ...
        if($this->db->count_all_results()!=0){
            ...
            if($this->db->count_all_results()==0){ 
                ...
                $this->db->update('pemeliharaan_meter_air', $data);
                ...
                if($tmpDiff){
                    $this->m_log->log_save('pemeliharaan_meter_air',$dataTmp['id'],'Edit',$diff);
                    return 'success';
	...
	public function delete($dataTmp)
    {
        ...
        if ($this->db->count_all_results() != 0) {
                        ...
                        $before = $this->get_log($dataTmp['id']);
                        $this->db->where('id', $dataTmp['id']);
                        $this->db->update('pemeliharaan_meter_air', ['delete' => 1]);
                        $after = $this->get_log($dataTmp['id']);
