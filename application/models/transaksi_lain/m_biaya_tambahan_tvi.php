     ...
     public function save($dataTmp)
    {
       ...
       echo "</PRE>";
        ...
        [
            ...
            'total_tagihan' => $this->m_core->currency_to_number($dataTmp['total_tagihan_biaya']),
         ...
         $dataBiayaTambahanDetail = [
            ...
            [
                ...
                'tanggal' =>  $dataTmp['tanggal_tagihan'],
                'project_id'    => $project->id,
                'jenis_paket_id' => $dataTmp['jenis_paket_id'],
                'total' => $this->m_core->currency_to_number($dataTmp['total_tagihan_biaya']),
                'registrasi_id' =>$dataTmp['registrasi_id'],
                'status_bayar' => 0,
                'flag_type' => 4,
                'active' => 1,
        ...
        if ($this->db->count_all_results() == 0) {
            ...
            for ($i = 0; $i < count($dataTmp['item']); ++$i) {
                ...
                $description[$i] = $dataTmp['description'][$i];
                $satuan[$i] = $dataTmp['satuan'][$i];
                $total[$i] = $dataTmp['total'][$i];
                ...
                $dataBiayaTambahanDetail2 = [
                    ...
                    'total' => $this->m_core->currency_to_number($dataTmp['total'][$i]),
    ...
    public function get_log($id)
    {
        ...
        $query = $this->db->query("
            SELECT  
                    ...
                    t_tvi_biaya_tambahan_detail.item    as item,
                    t_tvi_biaya_tambahan_detail.quantity  as volume,
                    t_tvi_biaya_tambahan_detail.harga_satuan  as harga_satuan,
                    t_tvi_biaya_tambahan_detail.satuan  as satuan,
                    t_tvi_biaya_tambahan_detail.total  as total,
                    t_tvi_biaya_tambahan_detail.description  as keterangan,
                    ...
                    t_tvi_tagihan.registrasi_id  as registrasi_id,
                    ...
                    case when t_tvi_tagihan.active = 0  then 'Tidak Aktif' else 'Aktif' end as tagihan_aktif,
                    case when t_tvi_tagihan.[delete] = 0 then 'Tidak di Hapus' else 'Terhapus' end as tagihan_delete
        ...
        foreach ($row as $v) {
            if (!array_key_exists('nomor_tagihan_biaya_tambahan', $hasil)) {
                ...
                $hasil['id_tagihan'] = $v['id_tagihan'];
                $hasil['kode_bill'] = $v['kode_bill'];
                $hasil['tanggal_tagihan'] = $v['tanggal_tagihan'];
                $hasil['jenis_paket_id'] = $v['jenis_paket_id'];
                $hasil['total'] = $v['total'];
                $hasil['registrasi_id'] = $v['registrasi_id'];
                $hasil['status_bayar'] = $v['status_bayar'];
                $hasil['tanggal_mulai'] = $v['tanggal_mulai'];
                $hasil['tanggal_berakhir'] = $v['tanggal_berakhir'];
                $hasil['tagihan_aktif'] = $v['tagihan_aktif'];
                $hasil['tagihan_delete'] = $v['tagihan_delete'];
            ...
            $hasil[$i.' item'] = $v['item'];
            $hasil[$i.' quantity'] = $v['volume'];
            $hasil[$i.' harga_satuan'] = $v['harga_satuan'];
            $hasil[$i.' satuan'] = $v['satuan'];
            $hasil[$i.' total'] = $v['total'];
