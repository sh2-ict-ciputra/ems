...
class m_pembayaran_tvi extends CI_Model
{
    ...
    public function getTagihan($id)
    {
        ...
        $query = $this->db->query("
         ...
         SELECT     
                   ...
                   CASE  t_tvi_tagihan.flag_type 
                   WHEN 1 then 'Biaya Pemasangan' 
                   WHEN 2 then 'Biaya Administrasi' 
                   WHEN 3 then 'Harga Paket' 
                   WHEN 4 then 'Biaya Tambahan' 
                   END as flag_type, 
                   t_tvi_tagihan.[delete] as delete_tagihan,
                   t_tvi_tagihan.tanggal as tanggal_tagihan,
                   paket_tvi.name as jenis_paket,
                   t_tvi_tagihan.total as total_tagihan                


        FROM t_tvi_tagihan
        LEFT JOIN paket_tvi on paket_tvi.id = t_tvi_tagihan.jenis_paket_id
        where t_tvi_tagihan.registrasi_id = $id and t_tvi_tagihan.project_id = $project->id and t_tvi_tagihan.status_bayar = 0 and t_tvi_tagihan.flag_type !=4
    ...
    public function getBiayaTambahan($id)
    {
        ...
        FROM t_tvi_tagihan
        LEFT JOIN t_tvi_biaya_tambahan on t_tvi_biaya_tambahan.registrasi_id = t_tvi_tagihan.registrasi_id
        LEFT JOIN paket_tvi on paket_tvi.id = t_tvi_tagihan.jenis_paket_id
        where t_tvi_biaya_tambahan.id = $id and t_tvi_tagihan.project_id = $project->id and t_tvi_tagihan.status_bayar = 0 and t_tvi_tagihan.flag_type=4
        order by t_tvi_tagihan.id asc
    ...
    public function save($dataTmp)
    {
        ...
        if ($this->db->count_all_results() == 0) {
            ...
            echo '<PRE>';
            print_r($dataTmp['id_tagihan']);
            echo '</PRE>';



            if (isset($dataTmp['id_tagihan'])) {
                    for ($i = 0; $i < count($dataTmp['id_tagihan']); ++$i) {

                        $id_tagihan[$i] = $dataTmp['id_tagihan'][$i];
                        $sisa[$i] = $dataTmp['sisa'][$i];


                        $dataTagihan2 =
                            [
                                'registrasi_id' =>  $dataTmp['registrasi_id'],
                                'total' =>  $this->m_core->currency_to_number($sisa[$i]),  
                                'status_bayar' => 1,
                                'active' => 0,
                                'delete' => 0,
                            ];

                        $beforetagihan = $this->get_log($id);



                        $this->db->where('id',  $id_tagihan[$i]);
                        $this->db->update('t_tvi_tagihan', $dataTagihan2);


                        $aftertagihan = $this->get_log($id);
                    }
                }
            ...
            if ($dataLog or $tmpDiff or $tmpDiff2) {
                ...
                //   $tmpDiff2?$this->m_log->log_save('t_tvi_tagihan', $id, 'Edit', $diff2):'';
    ...
    public function get_log($id)
    {
        ...
        $query = $this->db->query("
            SELECT 
                   ...
                   t_tvi_pembayaran.no_fisik_kwitansi as no_fisik_kwitansi,
                   t_tvi_pembayaran.total_bayar as total_bayar,
                   t_tvi_pembayaran.keterangan as keterangan_bayar,
                   t_tvi_pembayaran.nomor_pembayaran as nomor_pembayaran,
                   t_tvi_pembayaran.total_tagihan as total_tagihan,
                   t_tvi_pembayaran.sisa_tagihan as sisa_tagihan,
                   t_tvi_pembayaran.discount as discount,
                   t_tvi_pembayaran.nomor_tagihan as nomor_tagihan,
                   t_tvi_pembayaran.posting as posting,


                   t_tvi_tagihan.kode_bill as kode_bill,
                   t_tvi_tagihan.tanggal as tanggal_tagihan,
                   t_tvi_tagihan.jenis_paket_id as jenis_paket_id,
                   t_tvi_tagihan.total as total_tagihan,
                   t_tvi_tagihan.status_bayar as status_bayar,
                   t_tvi_tagihan.tanggal_mulai as tanggal_mulai,
                   t_tvi_tagihan.tanggal_berakhir as tanggal_berakhir,                  
    ...
    public function get_log_pembayaran($id)
    {
        $this->load->model('m_core');
        $project = $this->m_core->project();
