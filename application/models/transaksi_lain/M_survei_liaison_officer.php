...
class m_survei_liaison_officer extends CI_Model
{
    ...
    public function save2($dataTmp)
    {
        ...
        $hasil_survei2 = ($dataTmp['hasil_survei2']=='on')?1:0;

        if ($this->db->count_all_results() != 0 && !empty($hasil_survei2) ) {
            $this->db->where('nomor_registrasi', $dataTmp['nomor_registrasi'])->where('id !=', $dataTmp['id']);
            $this->db->from('t_loi_registrasi');
            // validasi double
            if ($this->db->count_all_results() == 0) {
                ...
                if (!empty($dataTmp['nama_item']))
                {
                    for($i= 0;$i<count($dataTmp['nama_item']);$i++) {            
                        ...
                        $volume[$i] =  $this->m_core->currency_to_number($dataTmp['volume'][$i]);
                        $satuan[$i] = $dataTmp['satuan'][$i];
                        $bukti_survei[$i] = $bukti_survei[$i] = (count($dataTmp['bukti_survei'])=='')?NULL:$dataTmp['bukti_survei'][$i];;
                        $dataItem = [
                            't_loi_registrasi_id'   => $dataTmp['id'],
                            'nama_item'             => $nama_item[$i],
                            'volume'                => $volume[$i],
