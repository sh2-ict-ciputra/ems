		...
		$("#btn-load-blok").click(function() {
			...
			if ($("#blok").val() != null) {
				$.ajax({
					...
					url: "<?= site_url() ?>/Transaksi/P_belum_tertagih/ajax_get_unit2",
					dataType: "json",
					success: function(data) {
						...
						for (var i = 0; i < data.length; i++) {
							...
							$("#tbody_unit").append(
								...
								"<td class='col-md-1'><a href='<?= site_url(); ?>/Transaksi/P_belum_tertagih/unit_detail?id=" + data[i].unit_id + "&periode=<?= $get_periode ?>' class='save-row btn btn-primary' unit_id='" + data[i].id + "'>                                        <i class='fa fa-book'></i></a></td>" +
								"</tr>");
						}
