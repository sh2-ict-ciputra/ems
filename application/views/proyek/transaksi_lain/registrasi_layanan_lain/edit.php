...
<div style="float:right">
	<h2>
		...
		<button class="btn btn-success" onClick="window.location.href='<?= site_url() ?>/transaksi_lain/P_registrasi_layanan_lain/edit?id=<?=$this->input->get('id')?>'">
		...
		<div class="col-md-6 div-unit">
			...
			<div class="col-md-9 col-sm-9 col-xs-12">
				<div class="form-group">
					<select id='pilih_unit' class='form-control select2' name="unit_id" disabled require>
					</select>
				</div>
			</div>
		</div>
		...
		<div class="col-md-12 div-non-unit" style="margin-top:10px" hidden>
			<div class="col-md-6">
				...
				<div class="col-md-7 col-sm-9 col-xs-12">
					<select name="customer_id" id="pilih_customer" class="form-control select2 non_unit" disabled>
			...
			<div class="col-md-6">
				...
				<div class="col-md-7 col-sm-9 col-xs-12">
					<div class="form-group">
						<select id='pilih_unit_virtual' name="unit_virtual_id" class=' form-control select2' disabled>
						</select>
					</div>
				</div>
				<div class="col-md-2 col-sm-9 col-xs-12">
			...
			<div class="col-md-6">
				<div class="form-group">
					...
					<div class="col-md-9 col-sm-9 ">
						<select id="pilih_paket_service" class="form-control" name="paket_service" require disabled>
							<option></option>
				...
				<div class="form-group">
					...
					<div class="col-md-3 col-sm-3" style="padding-right:0px">
						<input id="periode-awal" name="periode_awal" type="text" class="hitung-periode form-control text-center datetimepicker" placeholder="m / y" value="<?= date('d/m/Y') ?>" readonly disabled>
					</div>
					<div class="col-md-3 col-sm-3" style="padding-right:0px">
						<input id="periode-akhir" name="periode_akhir" type="text" class="hitung-periode form-control text-center datetimepicker" placeholder="m / y" value="<?= date('d/m/Y') ?>" readonly disabled>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-md-9 col-sm-9 ">
						<input id="kuantitas" value="1" name="kuantitas" type="number" class="form-control text-right" placeholder="-- Input kuantitas --" style="50px;padding-right:70px" min=1 disabled>
...
<script type="text/javascript">
	...
	$(function() {
		...
		setTimeout(function(){
		<?php if($data->unit_id > 0):?>
			$("#pilih_unit").select2("trigger", "select", {
                    data: { id: "<?=$data->unit_id?>",text:"<?=$data->unit_name?>" }
            });
            $("#pilih_unit").val("<?=$data->unit_id?>").trigger("change");

		<?php else:?>
			$("#is_unit").trigger('click');
			$("#pilih_customer").select2("trigger", "select", {
                    data: { id: "<?=$data->customer_id?>",text:"<?=$data->customer_name?>" }
            });
            $("#pilih_customer").val("<?=$data->customer_id?>").trigger("change");
			setTimeout(function(){
			$("#pilih_unit_virtual").select2("trigger", "select", {
                    data: { id: "<?=$data->unit_virtual_id?>",text:"<?=$data->unit_name?>" }
            });
            $("#pilih_unit_virtual").val("<?=$data->unit_virtual_id?>").trigger("change");
			},1000);


		<?php endif;?>
		$("#is_unit").attr('disabled',true)
		<?php if($data_detail->paket_service_id):?>
            $("#pilih_paket_service").val("<?=$data_detail->paket_service_id?>").trigger("change");
		<?php endif;?>
		setTimeout(function(){
			<?php if($data_detail->periode_awal):?>
				$("#periode-awal").val("<?=$data_detail->periode_awal?>").trigger("change");
				$("#periode-akhir").val("<?=$data_detail->periode_akhir?>").trigger("change");	
			<?php endif;?>
			<?php if($data_detail->kuantitas):?>
				$("#kuantitas").val("<?=$data_detail->kuantitas?>").trigger("change");				
			<?php endif;?>
