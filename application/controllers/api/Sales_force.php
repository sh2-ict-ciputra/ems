    ...
    public function index_get($api_key=null,$type=null)
    {
        ...
        if($type == "bill"){
            ...
            $resultVA = $this->db
                        ...
                        ->get()->result();
            $result->va = (object)[];
            $resultApiKey = $this->db->select("apikey")
                                    ->from("pt_apikey")
                                    ->join("unit",
                                            "unit.pt_id = pt_apikey.pt_id")
                                    ->where("unit.id",$resultUnit->unit_id)
                                    ->get()->row();
            $resultApiKey = $resultApiKey?$resultApiKey->apikey:"";
            $va = (object)[];
            foreach ($resultVA as $key => $v) {
                if($v->name == 'BCA')
                    $va->{$v->name} = [
                        "tipe" => "midtrans",    
                        "va_number" => $v->va,
                        "logo" => base_url("images/my_ciputra/".$v->name."Logo.png")
                    ];
                else
                    $va->{$v->name} = [
                        "tipe" => "xendit",    
