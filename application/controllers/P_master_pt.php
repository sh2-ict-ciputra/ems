...
class P_master_pt extends CI_Controller
{
    ...
    public function edit(){
        $data = $this->m_PT->get_dbmaster($this->input->get("id"));
        $data = $data[0];
        $this->load->view('core/header');
        $this->load->model('alert');
		$this->alert->css();
        ...
        $this->load->view('core/top_bar', ['jabatan' => $GLOBALS['jabatan'], 'project' => $GLOBALS['project']]);
        $this->load->view('core/body_header', ['title' => 'Master > PT', 'subTitle' => 'Add']);
        $this->load->view('proyek/master/pt/edit',[
            "data" => $data,
        ]);
        $this->load->view('core/body_footer');
        $this->load->view('core/footer');
    ...
    public function ajax_save(){
        echo json_encode($this->m_PT->ajax_save($this->input->post()));
    }
